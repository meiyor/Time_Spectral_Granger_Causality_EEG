function reading_eeg_saved_MVGC(patient_path)
...
stacked_eeg_plot_struct_list(EEG_list, 1:size(X_sub_eeg.EEG_k.data,1), stage_names, 50, 50, [patient_path, '_stacked']);
...
for stages = 1:11 %% generate the GC estimation and outputs for all the stages S1-S11 
    ...
    MVGC_application(X_sub_eeg.EEG_k, [patient_path, '_S', num2str(stages)], 'custom', 20, freq_vector);
end
